...
class HL_LH_block_1(nn.Module):
    def __init__(self, in_feats, hidden_size, out_feats, encode_dim, num_heads,num_layers):
        ...
        self.lg_layer = EGATlayer(in_node_feats=self.bond_feature_size,in_edge_feats=self.angle_feature_size,out_node_feats=self.bond_feature_size,out_edge_feats=self.angle_feature_size,num_heads=self.num_heads)

        for _ in range(self.num_layers-1):
            self.lg_layers.append(self.lg_layer)


        self.g_layer = EGATlayer(in_node_feats=self.atom_feature_size,in_edge_feats=self.bond_feature_size,out_node_feats=self.atom_feature_size,out_edge_feats=self.bond_feature_size,num_heads=self.num_heads*2)
...
class HL_LH_block_2(nn.Module):
    def __init__(self, in_feats, hidden_size, out_feats, encode_dim, num_heads,num_layers):
        ...
        self.g_layer = EGATlayer(in_node_feats=self.atom_feature_size,in_edge_feats=self.bond_feature_size,out_node_feats=self.atom_feature_size,out_edge_feats=self.bond_feature_size,num_heads=self.num_heads) 

        for _ in range(num_layers):
            self.g_layers.append(self.g_layer)
        
        '''
        self.fg_layer = EGATlayer(in_node_feats=self.angle_feature_size,in_edge_feats=self.dihedral_feature_size,out_node_feats=self.angle_feature_size,out_edge_feats=self.dihedral_feature_size,num_heads=self.num_heads*2)
...
class HL_LH_block(nn.Module):
    def __init__(self, in_feats, hidden_size, out_feats, encode_dim, num_heads, num_layers):
        ...
        self.lh_dir = HL_LH_block_2(in_feats = self.in_feats, hidden_size=self.hidden_size, out_feats=self.out_feats, encode_dim=self.encode_dim, num_heads=self.num_heads,num_layers=self.num_layers)

        self.block = graph_block(in_feats = self.in_feats, hidden_size=self.hidden_size, out_feats=self.out_feats, encode_dim=self.encode_dim, num_heads=self.num_heads,num_layers=self.num_layers)

        self.block_layers.append(self.hl_dir)
        self.block_layers.append(self.lh_dir)
        #self.block_layers.append(self.block)
...
class RBF_GAT(nn.Module):
    def __init__(self, in_feats, hidden_size, out_feats, encode_dim, out_dim, tras_med, num_blcok,num_heads,num_layers,activation=swish):
        ...
        layers4 = [
            nn.Linear(self.encode_dim[0], self.hidden_size),
            nn.Sigmoid(),
            #self.activation,
            nn.Linear(self.hidden_size, self.out_dim),
            nn.Sigmoid()
