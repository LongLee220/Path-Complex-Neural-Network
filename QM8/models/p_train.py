...
from sklearn.metrics import balanced_accuracy_score,r2_score,mean_squared_error,mean_absolute_error,mean_absolute_percentage_error
from sklearn.preprocessing import MinMaxScaler
from sklearn import metrics
from models.PGAT import HL_GAT,LH_GAT,LHL_GAT,HLH_GAT,EGAT,HL_LH_GAT
from models.hl_gat import RBF_GAT
...
from one_hot import mae_complex
from A_D import dis_ana
...
def creat_data(datafile, encoder_atom,encoder_bond,encode_two_path,encode_tree_path,force_field,batch_size,train_ratio,vali_ratio,test_ratio):
    ...
    if is_file_in_directory(directory_path, target_file_name):
        ...
        if datasets == 'qm9':
            ...
            dataset_dc = dc.molnet.load_qm8()
            tasks, datasets, transformers = dataset_dc
            ...
            for dataset in datasets:
                labels, smiles = dataset.y,dataset.ids
                labels_list.append(torch.tensor(labels))
                smiles_list.extend(smiles)

            all_labels = torch.cat(labels_list, dim=0)
            
            df_labels = pd.DataFrame(all_labels, columns=get_qm8_task_names())
        ...
        for i in range(len(smiles_list)):
            ...
            Graph_list = tree_complex(smiles, encoder_atom,encoder_bond,encode_two_path,encode_tree_path,force_field)
...
def train(model, device, target, train_loader, valid_loader, optimizer, epoch, resent,pooling):
    model.train()
    ...
    for labels, g_feats,lg, gg, fg in train_loader:
        ...
        train_label_pridct_tensor = model(g_feats, lg, gg, fg, device = device, resent = resent,pooling=pooling)
    ...
    if isinstance(loaded_valid_loader, list):
        ...
        total_loss_val = 0.0
        for labels, g_feats, lg, gg, fg in valid_loader:
            ...
            label_pridct_tensor = model(g_feats, lg, gg, fg, device = device, resent = resent,pooling=pooling)
...
def predicting(model, device,target, data_loader, min_val, max_val,resent,pooling,datafile,out_dim):
    ...
    if datafile  in ['qm8','qm9']:
        ...
        with torch.no_grad():
            for labels, g_feats, lg, gg, fg in data_loader:
                ...
                output = model(g_feats, lg, gg, fg, device = device, resent = resent,pooling=pooling).cpu()
    ...
    if datafile in['qm9','qm8','qm7']:
        ...
        y_pred = undo_transforms(predict_value_tensor, transformers)
        y_true = undo_transforms(predict_value_tensor, transformers)
...
if __name__ == '__main__':
    ...
    encoder_bond_dim = {"dim_14":14,"dim_26":26}
    encode_two_path_dim = {"dim_8":8,"dim_10":10}
    encode_tree_path_dim ={"dim_6":6,"dim_15":15}
    ...
    min_val, max_val = creat_data(datafile, encoder_atom, encoder_bond, encode_two_path,encode_tree_path, batch_size,force_field,train_ratio,vali_ratio,test_ratio)
    ...
    if model_select == 'hl':
        model = HL_GAT(in_feats=10, hidden_size = 64, out_feats=64,encode_dim=encode_dim)
    if model_select == 'lh':
        model = LH_GAT(in_feats=10, hidden_size = 64, out_feats=64,encode_dim=encode_dim)
    if model_select == 'lhl':
        model = LHL_GAT(in_feats=10, hidden_size = 64, out_feats=64,encode_dim=encode_dim)
    
    if model_select == 'hlh':
        model = HLH_GAT(in_feats=10, hidden_size = 64, out_feats=64,encode_dim=encode_dim,num_blcok=num_blcok)
    
    if model_select == 'hllh':
        model = HL_LH_GAT(in_feats=10, hidden_size = 64, out_feats=64,encode_dim=encode_dim, out_dim = out_dim).to(device)
    
    if model_select == 'egat':
        model = EGAT(in_feats=10, hidden_size = 64, out_feats=64,encode_dim=encode_dim, out_dim = out_dim).to(device)
    
    if model_select =='rbf_gat':
        model = RBF_GAT(in_feats=10, hidden_size = 64, out_feats=64,encode_dim=encode_dim, out_dim = out_dim,tras_med = tras_med).to(device)
    ...
    for i in range(iter):
        ...
        for epoch in range(NUM_EPOCHS):
            train_loss,vali_loss = train(model, device, target,loaded_train_loader, loaded_valid_loader,optimizer, epoch + 1, resent, pooling)
            ...
            if MAE < best_MAE:
                ...
                formatted_number = "{:.4f}".format(MAE)
    ...
    # 提取横坐标和纵坐标数据
    '''
    keys = list(train_loss_dic.keys())
    values1 = list(train_loss_dic.values())
    values2 = list(vali_loss_dic.values())

    # 绘制曲线图
    plt.plot(keys, values1, label='train_loss', marker='o')
    plt.plot(keys, values2, label='vali_loss', marker='s')
    
    # 添加标签和标题
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.title('train_loss & vali_loss')
    # 添加图例
    plt.legend()
    # 保存图形到文件（以PNG格式）
    plt.savefig('qm7.png')
